...
**This is the first tool. Apply it the moment you open any new function, Msg handler, keeper method, or CosmWasm entry point — before you reason about anything else.** Code you have not Feynman'd is code you have not actually understood.

When you read code, STOP and ask: "Can I explain what this function does to someone who doesn't know Cosmos or Rust?"

Try it. In plain words. The places where your explanation gets fuzzy — where you reach for `sdk.Context` / `SubMsg` / `BankKeeper` instead of plain meaning — are where you're papering over an assumption. That's where bugs hide.

Example: you read `k.bank.SendCoins(ctx, moduleAddr, recipient, coins)` and your explanation comes out as "it sends coins." That's not Feynman. Feynman is: "it takes tokens the module is holding and moves them to whoever the message named as recipient." Now keep going: who can name that recipient? Is the module actually holding those tokens, or is it minting? Your plain-English explanation breaks. Bug.
...
Example: `for id, coin := range assets { k.Process(ctx, id, coin) }`
- Why range over the map? → to process every asset.
- Why not sort the keys? → the developer assumed map order does not matter.
- What happens when two validators process in different orders and emit events or write state? → **different state roots. Chain halt.**
...
- Opening any new function, Msg, keeper method, or entry point → **Feynman** (always — before anything else)
