...
- `SKILL.md`
- `references/sandbox.md` — `@ton/sandbox`, Blueprint, `func-js` / `tact`
  commands for this worker image
- `references/poc.md`
...
rewrites the tests and examples for TON contracts — sender / Jetton wallet
authority, seqno replay, bounce handlers, `recv_external`, unreachable
internal helpers.
...
The `ton-auditor` skill in `../ton-auditor/` is an original adaptation of
Pashov `solidity-auditor` under MIT, with ToB TON pattern files under
CC-BY-SA-4.0 — see its own `ATTRIBUTION.md`.
