---
name: ton-auditor
description: Security audit of TON FunC/Tact code while you develop. Trigger on "audit", "check this contract", "review for security". Modes - default (full repo) or a specific filename.
---
...
**Exclude pattern:** skip directories `test/`, `tests/`, `wrappers/` and
`node_modules/`. Skip files under those trees even if they end in `.fc`,
`.func` or `.tact`.
...
a. Bash `find` for in-scope FunC/Tact files per mode selection. Example:

```
find . \( -name '*.fc' -o -name '*.func' -o -name '*.tact' \) \
  ! -path '*/test/*' ! -path '*/tests/*' ! -path '*/wrappers/*' \
  ! -path '*/node_modules/*' ! -path '*/.audit-*/*'
```
...
1. `{bundle_dir}/source.md` — ALL in-scope `.fc` / `.func` / `.tact` files, each with a `### path` header and fenced code block (fence language `func` or `tact` to match the file).
2. Agent bundles = `source.md` + agent-specific files:

| Bundle                | Appended files (relative to `{resolved_path}`)                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent-1-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/math-precision-agent.md` + `ton-patterns/integer-as-boolean.md` + `hacking-agents/shared-rules.md` |
| `agent-2-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/access-control-agent.md` + `ton-patterns/replay-bounce-opcode.md` + `hacking-agents/shared-rules.md` |
| `agent-3-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/economic-security-agent.md` + `ton-patterns/fake-jetton.md` + `hacking-agents/shared-rules.md` |
| `agent-4-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/execution-trace-agent.md` + `ton-patterns/replay-bounce-opcode.md` + `hacking-agents/shared-rules.md` |
...
| `agent-8-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/asymmetry-agent.md` + `ton-patterns/replay-bounce-opcode.md` + `hacking-agents/shared-rules.md` |
| `agent-9-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/boundary-agent.md` + `ton-patterns/forward-ton-gas.md` + `hacking-agents/shared-rules.md` |
...
Each bundle = source.md + SOP + specialty + (TON pattern refs where listed) + shared-rules. Agents read the bundle; no Read/Grep needed for the initial scan. Targeted Read/Grep allowed for cross-file investigation.
...
out-of-scope context (wrappers/, test/, tests/, node_modules/).
...
1. **Dedup.** Parse every FINDING and LEAD from the 12 agents. Group by `group_key` (Contract | function | bug-class). Exact-match first; merge synonymous bug_class within same (Contract, function). Keep best per group, number sequentially, annotate `[agents: N]`.
   ...
   **MANDATORY — Function isolation (HARD).** NEVER merge across different `function:` fields. Dedup only within (Contract, function). Different function = different bug. Second pass above stays WITHIN (Contract, function), never across. Treat `recv_internal` opcode branches (`if (op == op::transfer)`) as the **function** for grouping — use the opcode / Tact `receive(Foo)` name, not a single `recv_internal` bucket for the whole contract.
   ...
   **MANDATORY — Fix preservation (HARD GATE).** Before writing merged `fix:` on a multi-finding (Contract, function):
   ...
   2. Group by ADD-lines (`+` lines, or equivalent `throw_unless` / `require` / assignment).
   3. Distinct if ADD-lines differ in: called function/expression (e.g., `throw_unless(equal_slices(sender, owner))` vs `throw_unless(seqno == stored_seqno)`), check direction (validate/restrict/ban), or checked parameter.
...
2. **Gate.** Run each deduped finding through the four gates in `judging.md` (no skip, no reorder, no revisit after verdict).
   ...
   **Single-pass:** every relevant code path ONCE in fixed order (`recv_external` / `init` → `recv_internal` opcode dispatch → transfer / notify → withdraw / bounce handler). One-line verdict: `BLOCKS` / `ALLOWS` / `IRRELEVANT` / `UNCERTAIN`. `UNCERTAIN = ALLOWS`. Commit, no re-examination.
