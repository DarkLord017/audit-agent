...
- `SKILL.md`
- `references/cargo.md` — offline `cargo test` flags for this worker image
- `references/poc.md`
- `references/solana-lints.md`
...
rewrites the tests and examples for Solana programs — PDA authority,
upgrade authority, missing `is_signer`, `/// CHECK:` accounts, tests
under `tests/` / `target/`.
...
The `solana-auditor` skill in `../solana-auditor/` is an original
adaptation of Pashov `solidity-auditor` under MIT, with ToB Solana
pattern files under CC-BY-SA-4.0 — see its own `ATTRIBUTION.md`.
