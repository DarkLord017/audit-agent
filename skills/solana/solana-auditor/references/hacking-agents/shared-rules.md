...
Read the whole bundle once at the start. The bundle contains all in-scope source. Use Read/Grep only for cross-file searches or out-of-scope context (target/, mocks/, test/, tests/) — do not re-read in-scope files for the initial scan.
...
When matching instruction names, check the Anchor handler, the `#[derive(Accounts)]` struct, and any native `process_instruction` match arm. `contract:` is the program module; `function:` is the instruction.
...
|---|---|---|
| You open a new function or instruction to read | `[Feynman: <name>]` | Explain what it does in plain English — no Solana jargon, no `invoke`/`PDA`/`bump`/`AccountInfo`/`CPI`/etc. Use as many sentences as you need until the explanation is solid. If your wording slips back to jargon, you're papering over an assumption — keep going. Wherever your plain-English explanation gets fuzzy or you have to reach for a Solana term to keep it accurate, mark that spot — that is where bugs hide. |
...
| A code path reads as clean / a constraint looks sufficient / a guard looks correct | `[Inversion: <function>]` | Three concrete attacker moves that attempt to defeat the path. Specific pubkeys/amounts/account substitutions, not abstractions. |
...
When you find a bug in one program, **weaponize that pattern across every other program in the bundle.** Search by instruction name AND by code pattern. Finding an unvalidated `token_program` in `vault::withdraw` means you check every other `invoke` — missing a repeat instance is an audit failure.

After scanning: escalate every finding to its worst exploitable variant (DoS may hide fund theft). Then revisit every instruction where you found something and attack the other branches.

## Do not report

Admin-only instructions doing admin things. Standard DeFi tradeoffs (MEV, rounding dust). Self-harm-only bugs. "Upgrade authority can rug" without a concrete mechanism beyond the documented upgrade key.
