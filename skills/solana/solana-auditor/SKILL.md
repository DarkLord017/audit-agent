---
name: solana-auditor
description: Security audit of Solana/Anchor programs while you develop. Trigger on "audit", "check this program", "review for security". Modes - default (full repo) or a specific filename.
---
...
**Exclude pattern:** skip directories `target/`, `mocks/`, `test/`, `tests/`
and files matching `*test*.rs`, `*_test.rs`, `*Test*.rs`, `*Mock*.rs`,
`**/tests/**`, `**/test/**`.
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent-1-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/arbitrary-cpi-agent.md` + `hacking-agents/shared-rules.md`                             |
...
| `agent-8-bundle.md`   | `source.md` + `senior-auditor-sop.md` + `hacking-agents/pda-bump-agent.md` + `hacking-agents/shared-rules.md`                                  |
...
out-of-scope context (target/, mocks/, test/, tests/).
...
1. **Dedup.** Parse every FINDING and LEAD from the 12 agents. Group by `group_key` (Contract | function | bug-class). Exact-match first; merge synonymous bug_class within same (Contract, function). Keep best per group, number sequentially, annotate `[agents: N]`. For Solana, `contract:` is the program module (or Anchor `#[program]` module) and `function:` is the instruction handler.
   ...
   **MANDATORY — Fix preservation (HARD GATE).** Before writing merged `fix:` on a multi-finding (Contract, function):
   ...
   2. Group by ADD-lines (`+` lines, or equivalent require/constraint/assignment).
   3. Distinct if ADD-lines differ in: called function/expression (e.g., `require_keys_eq!(token_program.key, &spl_token::ID)` vs `constraint = bump == vault.bump`), check direction (validate/restrict/ban), or checked parameter.
...
2. **Gate.** Run each deduped finding through the four gates in `judging.md` (no skip, no reorder, no revisit after verdict).
   ...
   **Single-pass:** every relevant code path ONCE in fixed order (initialize → deposit → withdraw → swap → liquidate → admin setters). One-line verdict: `BLOCKS` / `ALLOWS` / `IRRELEVANT` / `UNCERTAIN`. `UNCERTAIN = ALLOWS`. Commit, no re-examination.
