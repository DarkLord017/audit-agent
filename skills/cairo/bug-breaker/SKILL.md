---
name: bug-breaker
description: Takes a Cairo/Starknet audit report and decides which claimed bugs are real. Triages each finding, then proves the survivors with a Starknet Foundry (snforge) test. Trigger on "/bug-breaker", "break these findings", "verify this audit", "prove these bugs". Marks every finding verified, disputed or unverified.
---
...
Writing an snforge test is the most expensive thing you can do, and your
...
**3. Cross-check with Caracal if it is on PATH.** See
[references/caracal.md](references/caracal.md). It is **not** installed in
this image. Skip the tool and say so; do not try to `cargo install` it.
Triage still runs from the report and the source.
...
[references/scarb.md](references/scarb.md) to compile and
...
`/work` is a **1g tmpfs**. Do not `scarb clean` into huge `target/` trees
you do not need; run one test at a time with `--exact`. If a build blows
the disk, mark remaining findings UNVERIFIED rather than filling the
mount.
...
`Contract::function` · `unzipped/<real/path>.cairo:<line>` · Confidence in: <from input>
...
```cairo
// poc/tests/test_foo.cairo
<the COMPLETE test source, verbatim -- every line you actually ran,
 including imports and the test function. Not a summary, not an excerpt.
...
**Result**
```
<the relevant snforge output showing it demonstrated the bug>
```
...
`Contract::function` · `unzipped/path/file.cairo:88`
...
`Contract::function` · `unzipped/path/file.cairo:12`
...
<Caracal detectors that line up with a finding, or "Caracal not in image">
````
