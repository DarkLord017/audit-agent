---
name: vyper-auditor
description: Security audit of Vyper code while you develop. Trigger on "audit", "check this contract", "review for security". Modes - default (full repo) or a specific filename.
---
...
**Exclude pattern:** skip directories `mocks/`, `test/`, `tests/` and files
matching `*Test*.vy`, `*Mock*.vy` or `test_*.vy`.
...
out-of-scope context (interfaces/, lib/, mocks/, test/, tests/).
...
1. **Dedup.** Parse every FINDING and LEAD from the 12 agents. Group by `group_key` (Contract | function | bug-class). Exact-match first; merge synonymous bug_class within same (Contract, function). Keep best per group, number sequentially, annotate `[agents: N]`.
   ...
   **MANDATORY — Fix preservation (HARD GATE).** Before writing merged `fix:` on a multi-finding (Contract, function):
   ...
   3. Distinct if ADD-lines differ in: called function/expression (e.g., `assert msg.value == amount` vs `assert token != empty(address)`), check direction (validate/restrict/ban), or checked parameter.
...
2. **Gate.** Run each deduped finding through the four gates in `judging.md` (no skip, no reorder, no revisit after verdict).
   ...
   **Single-pass:** every relevant code path ONCE in fixed order (`@deploy`/`__init__` → setters → swap → mint → burn → liquidate). One-line verdict: `BLOCKS` / `ALLOWS` / `IRRELEVANT` / `UNCERTAIN`. `UNCERTAIN = ALLOWS`. Commit, no re-examination.
