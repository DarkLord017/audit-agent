...
Read the whole bundle once at the start. The bundle contains all in-scope source. Use Read/Grep only for cross-file searches or out-of-scope context (interfaces/, lib/, mocks/, test/, tests/) — do not re-read in-scope files for the initial scan.
...
When matching function names, check both `foo` and `_foo` (Vyper convention: `@internal` helpers are often prefixed with `_`). Entry points are `@external` (and `@deploy` / `__init__`). `@internal` is not directly callable.
...
|---|---|---|
| You open a new function or contract to read | `[Feynman: <name>]` | Explain what it does in plain English — no Vyper jargon, no `raw_call`/`extcall`/`unsafe_add`/`convert`/`@nonreentrant`/etc. Use as many sentences as you need until the explanation is solid. If your wording slips back to jargon, you're papering over an assumption — keep going. Wherever your plain-English explanation gets fuzzy or you have to reach for a Vyper term to keep it accurate, mark that spot — that is where bugs hide. |
...
| A code path reads as clean / an assert looks sufficient / a guard looks correct | `[Inversion: <function>]` | Three concrete attacker moves that attempt to defeat the path. Specific addresses/values/states, not abstractions. |
...
Admin-only functions doing admin things. Standard DeFi tradeoffs (MEV, rounding dust, first-depositor with a burned MINIMUM_LIQUIDITY). Self-harm-only bugs. "Admin can rug" without a concrete mechanism.
