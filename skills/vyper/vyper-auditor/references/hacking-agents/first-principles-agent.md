...
For every state-changing `@external`:

1. **Extract every assumption.** Values (balance is current, price is fresh), ordering (A ran before B), identity (this address is what we think), arithmetic (fits in type, nonzero denominator), state (`HashMap` entry exists, flag was set, no concurrent modification).
...
3. **Exploit the break.** Trace execution with the violated assumption. Identify corrupted `self.` storage and extract value from it.

## Focus areas

- **Stale reads.** Read a value, modify state (or `raw_call` out), reuse the now-stale value — exploit the inconsistency. Missing `@nonreentrant` is one way; a same-function cache is another.
- **Desynchronized coupling.** Two storage variables must stay in sync. Find the writer that updates one but not the other.
- **Boundary abuse.** Zero, max, first call, last item, empty `DynArray`, supply of 1 — find where the code degenerates.
- **Cross-function breaks.** Function A leaves state in configuration X. Find where function B mishandles X.
- **Assumption chains.** A assumes B validates. B assumes A pre-validated. Neither `assert`s — exploit the gap.
