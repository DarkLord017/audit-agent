---
name: bug-breaker
description: Takes a Vyper audit report and decides which claimed bugs are real. Triages each finding, then proves the survivors with a Titanoboa pytest. Trigger on "/bug-breaker", "break these findings", "verify this audit", "prove these bugs". Marks every finding verified, disputed or unverified.
---
...
Writing a Titanoboa test is the most expensive thing you can do, and your
...
is required or you will see nothing. Slither understands `.vy` files when
`vyper` is on PATH.
...
[references/titanoboa.md](references/titanoboa.md) to compile and
...
`Contract.function` · `unzipped/<real/path>.vy:<line>` · Confidence in: <from input>
...
**Result**
```
<the relevant pytest output showing it demonstrated the bug>
```
...
`Contract.function` · `unzipped/path/File.vy:88`
...
`Contract.function` · `unzipped/path/File.vy:12`
