...
here for Vyper contracts. Original: William Woodruff, ["Brocards for
...
results. "This `@external` is unusual" is not a finding.
...
replace the blueprint, call `set_implementation`, or set the fee to 100%.
...
needs. Can an ordinary user do this? If it needs `assert msg.sender ==
self.owner`, dismiss unless the report argues the owner is not meant to
have that reach.
...
**Test:** is the function `@external`, and does any real path get there?
Check for:

- `@internal` with no `@external` caller
...
- test and mock contracts — **out of scope**, and a common false positive
  (`**/test/**`, `**/tests/**`, `**/mocks/**`, `test_*.vy`)

Slither's `entry-points` printer answers most of this. See
[slither.md](slither.md). In Vyper, only `@external` (and `__default__`,
`@deploy` / `__init__`) are callable from outside.
...
Vyper's default overflow revert is spec, not a bug. `unsafe_*` wrapping
without a bounds story *is* a finding.
...
**Test:** does the README, docstring, or a comment say this? "Owner is
...
| 2 | Owner can set fee to 100% | DISPUTED | 2 | owner already replaces the blueprint |
| 3 | Reentrancy in `_sweep` | DISPUTED | 3 | `@internal`, no `@external` caller |
| 4 | Missing `@nonreentrant` on view | DISPUTED | 4 | `@view` cannot write storage |
