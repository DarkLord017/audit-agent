...
- `SKILL.md`
- `references/slither.md` — Slither commands for `.vy` in this worker image
- `references/titanoboa.md`
- `references/poc.md`
...
rewrites the tests and examples for Vyper contracts — `@external` entry
points, `self.owner` / upgrade authority, `@nonreentrant`, `raw_call`,
...
The `vyper-auditor` skill in `../vyper-auditor/` is an original adaptation
of Pashov `solidity-auditor` under MIT — see its own `ATTRIBUTION.md`.
