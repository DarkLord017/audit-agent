---
name: bug-breaker
description: Takes a Sui Move audit report and decides which claimed bugs are real. Triages each finding, then proves the survivors with a sui move test. Trigger on "/bug-breaker", "break these findings", "verify this audit", "prove these bugs". Marks every finding verified, disputed or unverified.
---
...
Writing a Move test is the most expensive thing you can do, and your
...
modules and functions, not files. You have the code and `grep`, so
resolve each one yourself:

```
grep -rn "fun withdraw" unzipped/
```
...
**3. Detect Sui vs Aptos.** Read every `Move.toml` under `unzipped/`.
Markers:

| Dialect | `Move.toml` signals |
|---|---|
| **Sui** (primary) | `Sui =`, `MystenLabs/sui`, `sui-framework`, `sui = "0x2"` |
| **Aptos** | `AptosFramework`, `AptosStdlib`, `aptos-labs/aptos-core`, `aptos_framework` |

If nothing matches, treat it as **Sui**. This image has the Sui CLI and a
vendored Sui framework; it has **no Aptos CLI**. Aptos packages cannot be
proved here — mark those findings **UNVERIFIED** and say why. Mixed trees:
prove the Sui packages, leave Aptos UNVERIFIED.

See [references/sui.md](references/sui.md).
...
[references/sui.md](references/sui.md) to compile and
...
`module::function` · `unzipped/<real/path>.move:<line>` · Confidence in: <from input>
...
```move
// poc/tests/foo_tests.move
<the COMPLETE test source, verbatim -- every line you actually ran,
 including the module header, uses, and test function. Not a summary,
 not an excerpt. This block is the only place the proof survives: the
 stage that converts this report to JSON has no tools and cannot open
 your files.>
```

**Result**
```
<the relevant `sui move test` output showing it demonstrated the bug>
```
...
`module::function` · `unzipped/path/file.move:88`
...
`module::function` · `unzipped/path/file.move:12`

**Claim** — <one sentence>

**Verification** — <why you could not prove it: compile failure, missing
dependency, Aptos package, out of budget>

---

## Dialect

<Sui or Aptos, which Move.toml decided it, and whether tests ran>
````
