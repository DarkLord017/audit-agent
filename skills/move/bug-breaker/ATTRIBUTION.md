...
- `SKILL.md`
- `references/sui.md` — `sui move test` commands for this worker image
- `references/poc.md`
...
rewrites the tests and examples for Sui Move — `AdminCap` / capability
authority, shared vs owned objects, PTB atomic composition, and
`public(package)` functions with no entry caller.
...
The `move-auditor` skill in `../move-auditor/` is separate third-party
work under MIT — see its own `ATTRIBUTION.md`.
