...
Save the report to `{project-name}-algorand-audit-report-{timestamp}.md` in the current working directory, where `{project-name}` is the repo root basename and `{timestamp}` is `YYYYMMDD-HHMMSS` at scan time.
...
| **Files reviewed**               | `approval.py` · `clear.teal`<br>`escrow.teal` · `swap.py` | <!-- list every file, 3 per line -->
...
- **<Title>** — `Contract.function` — Code smells: <missing RekeyTo assert, unbounded GroupSize, etc.> — <1-2 sentence description of the trail and what remains unverified>
